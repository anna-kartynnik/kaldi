  ...
  try {
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      if(scale_reader.HasKey(utt)) {
        //Matrix<BaseFloat> new_feats(features.Value());
        //ApplyFmllrTransform(new_feats, transform_reader.Value(utt));
        //feature_writer.Write(utt, new_feats);
        for (i = 0; i < features.NumRows(); i++) {
          for (j = 0; j < features.NumCols(); j++) {
            if (scale_reader.Value(utt)(i, j) < 1e-4) {
              features(i, j) = 0.0;
            } else {
              features(i, j) /= scale_reader.Value(utt)(i, j);
              features(i, j) = std::min(features(i, j), 2.0f);
            }
          }}
       } else {
         KALDI_ERR << "Utterance " << utt << " is not found in scale matrix.";
       }
      //for (i = 0; i < features.NumRows(); i++) {
      //  for (j = 0; j < features.NumCols(); j++) {
      //    if (scale_matrix(i, j) == 0.0) {
      //      features(i, j) = 0.0;
      //    } else {
      //      features(i, j) /= scale_matrix(i, j);
      //    }
      //  }
      //}
      if (subtract_mean) {
